...
结构上一般包含：页面主题 + 核心信息 + 顶部写什么 + 核心区域布局（几栏 / 几个模块 / 如何排） + 每个模块的要点 + 底部总结 / 收束 + 整体风格 + 明确的空间取舍。不要把所有 outline 字段机械罗列出来，要像真人在委托设计师一样自然说出来。
...
5. **不要在 query 里重复 HTML 机械规范**：`.wrapper` 结构、`#bg` / `#ct` 分层、ECharts 容器 id 命名、`{renderer:'svg'}`、`__pptxChartsReady` 计数器、`../assets/echarts.min.js` script 路径、伪元素 `<span>` 包裹、单层背景、图片 `../images/` 前缀 —— 这些统一由下游生成器的 system prompt 管理，rewriter 只负责**内容、版面、风格指引**。但 query 必须用自然语言提醒："所有可见内容必须在 1600×900 页面内、留足约 60px 边距，空间不够就减少装饰、压缩模块和缩短文案，不能让卡片或图片越过底部/边缘"。忽略机械规范规则的另一例外是 inherited_image 的具体相对路径（见下条），那个必须在 query 里显式写出来让生成器知道用哪张图。
